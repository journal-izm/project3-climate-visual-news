...
기상청 기후 공공데이터를 이용해 데이터 시각화 뉴스 제작 흐름을 보여주는 교육용 기준 구현입니다. 훈련생은 이 기후 주제를 그대로 제출하지 않고 사회 문제·기후환경·경제금융 범위에서 별도 주제를 선정합니다.

## 확정 범위

- 수행시간: 72시간 / 팀 4~6명
- 환경: Windows 11, VS Code, Python 3.13.15
- 기준 예제 기간: 2015~2024년(10개 연도)
- 기본 지역: 서울·부산·대구
- 기준 예제 지표: 평균기온, 최고기온, 강수량
...
- `videos/`: 약 1분 AI 뉴스 MP4

## 훈련생 주제 적용 기준

- 후보 주제를 2개 이상 비교하고 강사 승인을 받은 뒤 구현합니다.
- 기본 분석기간은 최근 10개 연도 이내입니다.
- 시계열·비교·분포·관계·지도 등 주제에 맞는 시각화·그래프·인포그래픽을 8종 이상 제작합니다.
- 공간 데이터가 없으면 지도 대신 히스토그램·박스플롯·산점도 등 적합한 분포 시각화를 사용합니다.
- 앱의 열 이름, 필터, 통계, 기사 문장과 테스트 예상값을 팀 주제에 맞게 변경합니다.
- 최종 AI 영상은 `outputs/videos/`에 저장하거나 앱에서 업로드해 재생합니다.
